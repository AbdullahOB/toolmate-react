...
export default function HeroChat() {
  ...
  const mobileAnimationVariants = useMemo(
    () => ({
      initial: isMobile || shouldReduceMotion ? { opacity: 0 } : { y: 20, opacity: 0 },
      animate: isMobile || shouldReduceMotion ? { opacity: 1 } : { y: 0, opacity: 1 },
      exit: isMobile || shouldReduceMotion ? { opacity: 0 } : { y: -10, opacity: 0 },
      transition: isMobile || shouldReduceMotion ? { duration: 0.2 } : { duration: 0.5 },
    }),
  ...
  const messageAnimationVariants = useMemo(
    () => ({
      initial: isMobile || shouldReduceMotion ? { opacity: 0 } : { opacity: 0, y: 18 },
      animate: isMobile || shouldReduceMotion ? { opacity: 1 } : { opacity: 1, y: 0 },
      transition: isMobile || shouldReduceMotion ? { duration: 0.15 } : { duration: 0.3 },
    }),
    [isMobile, shouldReduceMotion]
  );
...
useEffect(() => {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined' && window.innerWidth <= 768) {
      ...
      const handleScroll = () => {
        ...
        if (currentScrollPosition > lastScrollPosition.current + 50 && isMobileChatOpened) {
  ...
  const mateyOutput = useMemo(() => messages.filter((msg) => msg.sender === 'matey').slice(-1)[0], [messages]);
  ...
  const ProductCard = React.memo(({ product, idx }: { product: any; idx: number }) => {
    let bgGradient = '';
    ...
    if (isMobileChatOpened) {
      return (
        ...
        ref={chatContainerRef}
          ...
          {!isMobile && (
          <motion.div className='absolute top-3 right-3 opacity-20' animate={{ rotate: 360 }}>
        ...
        <div className='relative z-10' >
          <motion.div
            ...
            <motion.div className='text-center p-3 flex flex-col items-center relative z-10' >
              <div className='w-24 h-24 mb-1 flex items-center justify-center'>
                <motion.img
                  src={product.assets || '/placeholder.svg?height=64&width=64'}
                  alt={product.name}
          ...
          <motion.div className='mt-3' >
            <p className='font-bold text-gray-700 text-md w-32 text-center leading-tight' title={product.name}>
              {product.name}
            </p>
          </motion.div>
        </div>
        <motion.button
          ...
          <span className='relative z-10 flex items-center gap-1 justify-center'>
            <Sparkles size={14} className={isMobile ? '' : 'group-hover:animate-pulse'} />
            Tell me why
          </span>
        </motion.button>
      </motion.div>
...
>
  ...
  />
          ...
          <AnimatePresence mode='popLayout'>
            {messages.map((message) => (
              <motion.div
                ...
                <div className={`flex z-5 relative ${message.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
                          ...
                          <div className='p-2 pr-0 flex flex-col gap-2'>
                            {message.sender === 'matey' && (
                              <button
                                ...
                                {copiedMessageId === message.id ? "Too easy, it's copied!" : 'Copy this!'}
                              </button>
                            )}
                            <button
                  ...
                  <AnimatePresence>
                    {!isMobile && hoveredMessageId === message.id && message.sender === 'matey' && (
                      <motion.div
                        initial={{ opacity: 0, scale: 0.8 }}
                        animate={{ opacity: 1, scale: 1 }}
                        exit={{ opacity: 0, scale: 0.8 }}
                        className={`absolute ${
                          message.id === '1' && message.sender === 'matey' ? 'right-[5.5rem]' : 'right-10'
                        } top-3 ml-2`}>
                        <button
                        ...
                        <AnimatePresence>
                          {showDropdown === message.id && message.sender === 'matey' && (
                            <motion.div
                              ...
                              <p className='w-full px-4 py-2 text-left text-sm text-gray-700'>Rate this message</p>
                              <button
                                onClick={() =>
                                  handleDropdownAction('helpful', message.id, message.text, message.timestamp)
                                }
                                className={`w-full px-4 py-2 text-left text-sm ${
                                  feedbackAnimations[message.id] === 'helpful' ? 'text-green-500' : 'text-gray-700'
                              ...
                              <button
                                onClick={() =>
                                  handleDropdownAction('unhelpful', message.id, message.text, message.timestamp)
                                }
                                className={`w-full px-4 py-2 text-left text-sm ${
                                  feedbackAnimations[message.id] === 'unhelpful' ? 'text-red-500' : 'text-gray-700'
                                } hover:bg-red-50 hover:text-red-700 transition-colors flex items-center gap-2 relative`}>
                                <motion.div
                                  animate={
                                    feedbackAnimations[message.id] === 'unhelpful'
                                      ? {
                                          scale: [1, 1.3, 1],
                                          rotate: [0, -10, 10, 0],
                                        }
            ...
            {isTyping && (
              <motion.div key='typing-indicator' className='mb-6 flex justify-start'>
        ...
        {showPrompt ? (
          <div className='p-3 bg-white/70 border-t border-yellow/30 relative'>
            <div ref={scrollContainerRef} className='flex gap-3 md:flex-wrap overflow-x-auto'>
        ...
        {!showUploadButton && !subscriptionData && uploadCount >= MAX_FREE_UPLOADS && (
          <div className='text-center text-sm text-gray-500 mt-1 mb-2'>
            <span className='flex items-center justify-center gap-1'>
              <ImageIcon size={14} /> You've used all {MAX_FREE_UPLOADS} uploads.
            </span>
          </div>
        )}
        ...
        <AnimatePresence>
          {imagePreview && (
            <motion.div
              ...
              <motion.div className='relative bg-white p-4 rounded-lg border border-yellow/30 shadow-lg' layout>
                <motion.div className='flex flex-col items-center'>
                  <p className='text-sm text-gray-600 mb-2'>Your photo is ready to send!</p>
                  <img
        ...
        <div className='p-2 sm:p-4 bg-white border-t border-yellow/30'>
          <div className='flex items-center gap-3'>
            {(subscriptionData || (showUploadButton && uploadCount < MAX_FREE_UPLOADS)) && (
              <motion.button
  ...
  '
                    ...
                    <motion.div
                      ...
                      {suggestedPrompts.map((prompt, index) => (
                        <motion.div
                          ...
                          initial={messageAnimationVariants.initial}
                          animate={messageAnimationVariants.animate}
                          transition={{ ...messageAnimationVariants.transition, delay: isMobile ? 0 : index * 0.1 }}
            ...
            <motion.button
              ...
              disabled={(inputValue.trim() === '' && !imagePreview) || isCompletingText || isCompressingImage}>
              <Send size={18} />
            </motion.button>
          </div>
        </div>
