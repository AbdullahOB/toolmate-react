    ...
    if (typeof window !== "undefined" && window.innerWidth <= 768) {
      ...
      const handleViewportChange = () => {
        ...
        if (heightDifference < 100) {
          ...
          setTimeout(() => {
            scrollToTopAndReset();
          }, 210); // Small delay to ensure keyboard is fully closed
        }
      };
  ...
  const ProductCard = React.memo(
    ({ product, idx }: { product: any; idx: number }) => {
      ...
      return (
        <motion.div
          key={idx}
          className={`relative flex flex-col justify-between rounded-2xl p-4 flex-shrink-0 bg-gradient-to-br ${bgGradient} shadow-lg hover:shadow-xl transition-all duration-300 border border-white/50 backdrop-blur-sm min-w-[140px]`}
        >
          {!isMobile && (
            <motion.div
        ...
        <div className="bg-gradient-to-r from-yellow to-softYellow p-2 md:p-4 flex items-center gap-4">
          <div className="flex items-center gap-4">
            ...
            <div className="flex flex-col">
              ...
              <p className="text-black/70 text-start text-md">
