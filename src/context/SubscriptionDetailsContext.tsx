...
interface SubscriptionContextType {
  ...
  requestSubscriptionPause: (
    ...
    message: 'cancel' | 'suspend' | 'downgrade',
...
export const SubscriptionProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const requestSubscriptionPause = async (
    isDownGradeRequest: boolean,
    message: 'cancel' | 'suspend' | 'downgrade',
    downGradeDuration: number
  ) => {
    ...
    try {
      ...
      toast({
        title: 'Success',
        description: `${message} request sent successfully! ${message} takes effect from next billing cycle | ${new Date(
          subscriptionData.nextBillingCycle
        ).toDateString()}`,
        variant: 'success',
      });
