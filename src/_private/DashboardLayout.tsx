...
const DashboardLayout: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (userData.planAccess[0] && route.pathname !== '/manage-subscription') {
      console.log(
        route.pathname,
        'kkkkkkkkkkkkkkk',
        userData.planAccess[0] && route.pathname !== '/manage-subscription'
      );
  ...
  */
  ...
  useEffect(() => {
    const routeToNotShowSidebar = ['/manage-subscription'];
    function checkRoute() {
  ...
  // Commenting out loading and error states to just show UI
  /*
  if (!isLoaded) {
